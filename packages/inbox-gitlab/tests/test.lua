...
local function test_configures_the_refresh_interval()
	...
	assert(state:source_action("settings") == nil, "GitLab settings must not add an outer submenu")
	assert(state.configuration.actions[2].id == "refresh_interval", "GitLab refresh interval must be top-level")
	assert(state.configuration.actions[3].id == "merge_method", "GitLab merge method must be top-level")
	assert(state.configuration.actions[4].id == "merge_confirmation", "GitLab merge configuration must be top-level")
